...
export default {
  ...
  subscriptions: null,
  activate(state) {
    this.snappyshotAtomView = new SnappyshotAtomView(
      state.snappyshotAtomViewState
    );
  ...
  serialize() {
    return { snappyshotAtomViewState: this.snappyshotAtomView.serialize() };
  },
  generateSnapshot(foo: string) {
    ...
    if (editor = atom.workspace.getActiveTextEditor()) {
      ...
      let newFilePath = filePath[0] + "/__tests__/" + relativeFilePath;
      console.log("newFilePath: " + newFilePath);
      let file = new File(newFilePath, false);
      file.create().then(() => {
        file.write(snapshot).then(() => {
          alert("yeah!!!");
        });
      });
